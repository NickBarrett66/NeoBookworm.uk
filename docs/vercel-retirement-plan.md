...
**Status: COMPLETE (22 Jul 2026).** All six stages done and verified. Stages 1–4: all five email endpoints send via the Gmail API from the Cloudflare Worker; the `.co.uk` redirect no longer touches Vercel — this had already been stable in production for a couple of weeks before Stage 5 began. Stage 5b/5c: `api/` and `vercel.json` deleted, `VERCEL_BRIDGE_URL`/`BRIDGE_SECRET` Worker secrets removed. One deviation from the original plan: `api/vdg-lookup.js` + `api/_lib/vdg.js` (the VDG tyre-lookup used by `reg-test.html` for the TyreTrust mailshot) turned out to have **no** Cloudflare equivalent, unlike the plan's assumption that everything except the five email endpoints was already a dormant duplicate — ported to `worker/_lib/vdg.js` + `worker/routes/vdg-lookup.js` before deleting `api/`, verified with a real lookup (Vauxhall Astra, full tyre fitment data) once `VDG_API_KEY` was set as a Worker secret (the old Vercel value couldn't be pulled programmatically — `vercel env pull` returned every value blank for this project/account, a CLI quirk, not evidence the key itself was empty — Nick set it manually). Stage 5a: `bridge` DNS record deleted. Stage 5d: GitHub integration disconnected (`vercel git disconnect`), then the `neo-bookworm-uk` project deleted (`vercel project remove`) — confirmed gone from `vercel project ls`, and `bridge.neobookworm.uk` no longer resolves at all. Main site and TyreTrust both verified 200 throughout. Written 1 Jul 2026.
