This page replaces the original `outreach-templates.md` from the March 2026 build session. It covers all four segments (A, B, C, D) with initial emails, follow-ups, and the prompt guidance that goes into Agent 3 and Agent 6.

All templates are written to be used as **guidance for Claude**, not pasted verbatim. Claude fills in the personalisation blanks from the Agent 2 research. What these templates define is the **structure, tone, angle, and constraints** for each segment.
...
- **Email 1** — the initial outreach (sent by Agent 3)
- **Email 2** — the first (and for sole traders, final) follow-up (sent by Agent 6)
- **Email 3** — second follow-up for Ltd companies only (sent by Agent 6)
- **What NOT to do** — specific mistakes to avoid in this segment
- **Agent 3 prompt guidance** — the exact brief Claude receives
...
> Won't message again after this — just wanted to make sure you'd seen it.
> 
...
**Key requirements for sole traders:** Must include a natural "won't message again" line. Claude should write this as *"Won't message again after this"* or *"This is my last one, promise"* — not a formal statement.
...
> Won't keep chasing — just wanted to make sure you'd seen it.
> 
...
**Key requirement for sole traders:** Must include natural final-contact language in closing line.
...
> Won't chase again after this — just wanted to make sure you'd seen it.
> 
...
# Agent 3 and Agent 6 prompt guidance

This section contains the exact `segmentAngles` and guidance object to use when building Claude's prompt in both Agent 3 (initial email) and Agent 6 (follow-ups).

## segmentAngles for Agent 3 (Email 1)
...
## segmentFollowUpAngles for Agent 6
...
## Local search volume lookup

Embed this object in agent3-worker.js and agent6-worker.js. Used when Claude is instructed to include a local search volume data point.

```jsx
const localSearchVolumes = {
  "Plumber": "around 170",
...
// Usage in prompt:
// const searchVolume = localSearchVolumes[tradeCategory] || "hundreds of";
// Include in Agent 6 follow-ups only (not Email 1) — one data point, used once.
```
