...
| Item | Priority | Notes |
|---|---|---|
| Booking widget (Koalendar replacement) | Medium | Reusable appointment-booking iframe — Cloudflare Worker + D1 (`bookings` DB) + Google Calendar API + GW SMTP. Phase 1 in progress. Production URL: `https://neobookworm-booking.nickbarrett.workers.dev/hetyres`. `booking.neobookworm.uk` deferred until DNS moves to Cloudflare. Full session-by-session build doc: [docs/booking-widget-build.md](docs/booking-widget-build.md). |
