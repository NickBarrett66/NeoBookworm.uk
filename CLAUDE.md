...
| contact.html | Complete | Skip link + `<main id="main">`. Header/nav aligned with Home / How it works / Pricing. Three contact paths in accordion (single open): lazy intake iframe, inline Koalendar embed (`?embed=true` on `/u/nickbarrett` — swap to `/e/…` in Koalendar if embed fails), quick message via **Write a message** → **Send message**. Jump links + URL hash sync; click outside `#contact-options` collapses + clears hash; `noscript` hides expand buttons so forms stay usable without JS. Email POSTs to `api/contact.js`; set SMTP / `TO_EMAIL` in Vercel env vars |
