...
| guides.html | Complete | Expanded guides index. Two sections (prospect / client). "Start here" pinned hero card above Section 1. 11 new guide cards added (6 prospect, 5 client). One card renamed to customer-perspective framing. Contact email updated to nick@neobookworm.uk. |
...
| Run migration 0006 on D1 | High | `ALTER TABLE clients ADD COLUMN site_review_content TEXT;` — run in Cloudflare D1 console on neobookworm-enquiries DB before the new J2 audit pipeline will work. |
| Set ANTHROPIC_API_KEY in Vercel | High | Required by `api/run-site-audit.js` and `api/_lib/audit.js` (claude-sonnet-4-6). Add to Vercel project → Settings → Environment Variables. |
| Deploy CF Worker after intake.js change | High | `workers/landing-enquiry/src/intake.js` now triggers audit for J2 — `wrangler deploy` from `workers/landing-enquiry/`. |
