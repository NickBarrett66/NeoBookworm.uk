...
| Item | Priority | Notes |
|---|---|---|
| Booking widget (Koalendar replacement) | Medium | Reusable appointment-booking iframe at `booking.neobookworm.uk/:slug` — Cloudflare Worker + D1 (`bookings` DB) + Google Calendar API + GW SMTP. Phase 1 = single hardcoded `hetyres` client against the **`nick@neobookworm.uk` Workspace calendar** (Workspace = Internal OAuth consent → refresh tokens never expire). Full session-by-session build doc with tests + gates: [docs/booking-widget-build.md](docs/booking-widget-build.md). Not started — intended for Cursor execution. Embeds via iframe; fires `postMessage('booking-confirmed')` (HE Tyres page already listens). |
