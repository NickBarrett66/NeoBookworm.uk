...
| contact.html | Complete | Header/nav aligned with Home / How it works / Pricing. Three cohesive contact paths (full intake iframe, Koalendar call CTA, quick email form) with shared card styling, jump links (`#option-full`, `#option-call`, `#brief`). Email form POSTs to Vercel `api/contact.js`; set SMTP / `TO_EMAIL` in Vercel env vars |
