...
| plumbers-switch.html | Complete | Realigned to honest example-led model (May 2026). Sections: Hero (ownership/quality hook; “you don’t pay until it’s better than what you’ve got”; £299 fixed — £199 launch; primary CTA → free review; secondary → what I build) → Honesty block ("when not to switch" — one plain paragraph, no bullet list) → Four situations (C1 dead domain / C2 DIY template / C3 agency-managed / C4 functional-but-flawed — unlabelled, 2×2 card grid) → "What you’d be switching to" (six interactive modal cards + speed compare 24→96, ported from plumbers.html with updated eyebrow/heading) → Cost comparison table (ownership column added) → How switching works (3 steps: free review → build + preview before paying → domain move + no-pay-until-better) → About Nick (honest "building this up" line added) → FAQ (7 items: 2 new — "You’re new — why trust you?" and "Someone else manages my site — is this awkward?") → Form (option 1 = free site review as lead with confident framing; thank-you tailored per option). All 6 modal dialogs and modal JS ported. Leaflet area map. Form wiring preserved: source plumbers-switch-landing, Worker endpoint, ref=plumbers-switch intake redirect. Not in sitemap until go-live. |
